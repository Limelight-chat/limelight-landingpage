...
const SlideItem = ({
  ...
  return (
    ...
    >
      ...
      {/* EMOJIS */}
      {slide.emojis?.map((emoji, idx) => (
        <EmojiItem
          key={idx}
          emoji={emoji}
          start={start}
          end={end} // Assuming emojis animate within the slide duration, logic reused from original
          scrollYProgress={scrollYProgress}
          idx={idx}
        />
      ))}
...
const SlideImage = ({
  ...
  const imageStart = start + (itemIndex / totalImages) * ((end - start) * 0.7);
...
const EmojiItem = ({
  ...
  const emojiEnd = start + 0.35; // Logic copied from original loop
  ...
  const [rotation, setRotation] = React.useState(emoji.angle ?? 0);
  ...
  useAnimationFrame(() => {
    ...
    if (emoji.rotate) {
      setRotation((r: number) => (r + (emoji.anger ? 10 : 2)) % 360);
    }
  });

  const src = typeof emoji.src === "string" ? emoji.src : emoji.src.src;
